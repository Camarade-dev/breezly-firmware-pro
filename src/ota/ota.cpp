...
static void logPartitions(){
  ...
  if (run)  LOGD("OTA", "running  part: label=%s addr=0x%06x size=%u encrypted=%d",
      run->label,  run->address,  run->size, run->encrypted
  );
  if (next) LOGD("OTA", "next OTA part: label=%s addr=0x%06x size=%u encrypted=%d",
...
static bool fetchWithRetry(const String& u, String& out, int tries=4) {
  for (int i=1; i<=tries; ++i) {
    ...
    LOGI("OTA", "fetch fail try %d/%d", i, tries);
...
static bool httpGetToString(const String& url, String& out,
  ...
  if (!http.begin(wcs, url)) {
    LOGI("OTA", "http.begin FAIL: %s", url.c_str());
    return false;
  }
  ...
  LOGI("OTA", "manifest OK, %u bytes", (unsigned)out.length());
  return out.length() > 0;
}
...
void otaOnBootValidate(){
  ...
  if (pending){
...
#endif
      LOGI("OTA", "Rollback target: clear pending, mark valid");
      return;
    }
    ...
    if ((fail+1) >= MAX_FAILS){
      ...
      rb.end();
      LOGI("OTA", "Too many failed boots → rollback (record rolled_back_ver=%s)", CURRENT_FIRMWARE_VERSION);
#if ESP_IDF_VERSION_MAJOR >= 4
      esp_ota_mark_app_invalid_rollback_and_reboot();
#else
      LOGI("OTA", "Too many failed boots (IDF<4) → reboot factory");
      ...
      LOGI("OTA", "TEST: simulate failed boot %u/3 → reboot", (unsigned)(fail+1));
      delay(500);
      ESP.restart();
    }
#endif
...
static bool verifyManifestSignature(const String& canonical, const String& sigB64){
  LOGD("OTA", " verify sig: canonical_len=%u sigB64_len=%u", (unsigned)canonical.length(), (unsigned)sigB64.length());
  mbedtls_pk_context pk; mbedtls_pk_init(&pk);
  if (mbedtls_pk_parse_public_key(&pk, (const unsigned char*)OTA_PUBKEY_PEM, strlen(OTA_PUBKEY_PEM)+1) != 0){
    LOGI("OTA", "parse public key FAIL");
  ...
  mbedtls_pk_free(&pk);
  LOGI("OTA", "manifest signature %s", ok==0 ? "OK" : "INVALID");
  return ok==0;
}
                                                ...
                                                bool allowGithubFallback)
{
  ...
  if (WiFi.hostByName(host.c_str(), ip)) {
    LOGI("OTA", "DNS %s -> %s", host.c_str(), ip.toString().c_str());
  } else {
    LOGI("OTA", "DNS resolve FAIL for %s", host.c_str());
  ...
  auto beginWithHeaders = [&]() -> bool {
    ...
    if (!ok) {
      LOGI("OTA", "http.begin FAIL (host=%s port=%u path=%s)",
  ...
  for (int attempt = 1; attempt <= 4; ++attempt) {
    code = http.GET();
    LOGI("OTA", "BIN GET attempt %d -> code=%d (%s)",
    ...
    if (code < 0 || (code >= 500 && code < 600)) {
      ...
      if (!beginWithHeaders()) {
        LOGI("OTA", "http.begin FAIL on retry (host=%s)", host.c_str());
      }
      continue;
    }
  ...
  if (code != HTTP_CODE_OK) {
  LOGI("OTA", "HTTPClient GET failed, code=%d (%s)",
          code, http.errorToString(code).c_str());

  LOGI("OTA", "WiFi.status()=%d, RSSI=%d",
          (int)WiFi.status(), WiFi.RSSI());

  wifi_ap_record_t ap;
  if (esp_wifi_sta_get_ap_info(&ap) == ESP_OK) {
    LOGI("OTA", "AP: RSSI=%d, channel=%d",
            ap.rssi, ap.primary);
  }
  ...
  if (expectedSize && expectedSize > nextPart->size) {
    LOGI("OTA", "image too large: expectedSize=%u > partSize=%u",
  ...
  if (err != ESP_OK) {
    LOGI("OTA", "esp_ota_begin FAILED err=%d (%s)", (int)err, esp_err_to_name(err));
    http.end();
    g_otaInProgress = false;
    updateLedState(LED_BAD);
    return false;
  }
  LOGI("OTA", "esp_ota_begin OK (maxSize=%u)", (unsigned)nextPart->size);

  LOGD("OTA", "before buf malloc heap=%u", (unsigned)esp_get_free_heap_size());
  ...
  while (CH >= 512 && buf == nullptr) {
    ...
    if (!buf) {
      LOGI("OTA", "heap_caps_malloc(%u) FAIL -> try smaller", (unsigned)CH);
      CH /= 2;
    }
  }

  if (!buf) {
    LOGI("OTA", "malloc FAIL even at 512 bytes");
  ...
  LOGI("OTA", "chunk size = %u bytes", (unsigned)CH);
  ...
  while (true) {
    ...
    if (n < 0) {
      LOGI("OTA", "stream read ERROR (%d)", n);
      free(buf);
    ...
    if (n == 0) {
      if (!http.connected()) {
        LOGI("OTA", "HTTP disconnected, end of stream");
        break;
      }
      if (millis() - lastProgress > NO_PROGRESS_ABORT_MS) {
        LOGI("OTA", "no progress timeout");
        free(buf);
        mbedtls_sha256_free(&sha);
        esp_ota_abort(otaHandle);
        http.end();
    ...
    if (firstChunk) {
      firstChunk = false;
      LOGD("OTA", "first 16 bytes of image:");
    ...
    if (err != ESP_OK) {
      LOGI("OTA", "esp_ota_write FAIL err=%d (%s)", (int)err, esp_err_to_name(err));
      free(buf);
      mbedtls_sha256_free(&sha);
      esp_ota_abort(otaHandle);
      http.end();
    ...
    if ((millis() - lastLog) > 2000){
      lastLog = millis();
      LOGI("OTA", "written %u bytes...", (unsigned)written);
    }
    if ((millis() - lastLog) > 2000 && expectedSize) {
      lastLog = millis();
      int pct = (int)((written * 100ULL) / expectedSize);
      LOGI("OTA", "%d%% (%u/%u)", pct, (unsigned)written, (unsigned)expectedSize);
    }
  ...
  if (err != ESP_OK) {
    LOGI("OTA", "esp_ota_end FAIL err=%d (%s)", (int)err, esp_err_to_name(err));
    mbedtls_sha256_free(&sha);
    http.end();
    g_otaInProgress = false;
    updateLedState(LED_BAD);
  ...
  if (expectedSize && written != expectedSize) {
    LOGI("OTA", "size mismatch: written=%u expected=%u",
...
void checkAndPerformCloudOTA(){
  ...
  LOGI("OTA", "GET manifest: %s (forced=%d)", url.c_str(), (int)forced);
  ...
  if (jerr) { LOGI("OTA", "JSON parse error: %s", jerr.c_str()); return; }
  ...
  LOGI("OTA", "manifest: ver=%s size=%u rollout=%d force=%d min=%s",
          ver, (unsigned)expectedSize, rollout, (int)force, minver);
  LOGD("OTA", " model=%s channel=%s product=%s", model, channel, product);

  if (!ver[0] || !bin[0]) { LOGI("OTA", "champs manquants"); return; }
  ...
  if (n <= 0 || n >= (int)sizeof(canonical)) {
    LOGI("OTA", "canonical overflow");
    return;
  }
  if (!verifyManifestSignature(String(canonical), String(sigB64))) {
    LOGI("OTA", "Signature INVALID → refuse update");
    return;
  }


  if (String(model) != String("wroom32e")){ LOGI("OTA", "model mismatch (%s!=wroom32e)", model); return; }

  for (JsonVariant v : blocked){
    if (String(v.as<const char*>()) == String(CURRENT_FIRMWARE_VERSION)){
      LOGI("OTA", "version courante bloquée → force update");
      force = true;
    }
  }

  if (minver[0] && cmpSemver(String(CURRENT_FIRMWARE_VERSION), String(minver)) < 0){
    LOGI("OTA", "below min_version (%s < %s) → force update", CURRENT_FIRMWARE_VERSION, minver);
    force = true;
  }
  ...
  LOGD("OTA", "rollout=%d%% bucket=%u sensorId=%s", rollout, bucket, sensorId.c_str());
  LOGI("OTA", "rollout=%d%% bucket=%u", rollout, bucket);
  if (!force && bucket >= rollout){ LOGI("OTA", "skip by rollout"); return; }

    int cmp = cmpSemver(String(CURRENT_FIRMWARE_VERSION), String(ver));
  LOGI("OTA", "version cmp: current=%s target=%s -> %d", CURRENT_FIRMWARE_VERSION, ver, cmp);
  if (!force && cmp >= 0){
    LOGI("OTA", "déjà à jour (skip)");
    return;
  }
  ...
  if (rolledBack.length() > 0 && String(ver) == rolledBack){
    LOGI("OTA", "skip: version %s was rolled back (reject re-install)", ver);
    return;
  }
  ...
  LOGI("OTA", "UPDATE → %s", bin);
  LOGD("OTA", "before flash heap=%u", (unsigned)esp_get_free_heap_size());
  logPartitions();
  ...
  LOGI("OTA", "cooldown before BIN download: %lu ms", (unsigned long)cooldown);
  delay(cooldown);
  ...
  if (hostStart > 0 && pathIdx > hostStart) {
    ...
    String warmupBody;
    LOGI("OTA", "warmup GET to %s", warmupUrl.c_str());
  ...
  if (!ok) {
    LOGI("OTA", "download/flash failed");
    char otaFailCtx[120];
