...
| Flash flotte + journal EOL | `python scripts/flash_fleet.py --env esp32-wroom-32e-prod --jobs 10 --variant STD` (ou PREMIUM/B2B/B2B_PMS) ; 7 ports branchés | Build SUCCESS (~60 s) ; 7/7 upload OK ; journal EOL mis à jour (Variant, Port, external_id par appareil) | Échec build ou upload, journal non rempli | OK | Build 00:01:00 ; 7 ports (COM8, COM11, COM17, COM18, COM21, COM22, COM23) ; OK=7 FAIL=0 ; journal EOL mis à jour avec colonnes Variant/Port | 7 devices | — | 2026-02 |

*Post-upload / fleet provisioning* : `post_upload_register.py` (mono-device) et `flash_fleet.py` (mode flotte) utilisent désormais la même logique d’identité : lecture prioritaire de `BREEZLY_EXTERNAL_ID=PROV_xxxx` sur la série du device, puis fallback `esptool read_mac` si la ligne n’est pas reçue. Le provisioning backend part uniquement si `external_id` a bien été résolu ; sinon le port est marqué en échec dans le résumé final et dans le journal EOL. Optionnel : `pip install pyserial` pour fiabiliser la lecture série.
